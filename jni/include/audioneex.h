...
/// DO NOT define AUDIONEEX_API_EXPORT as it is reserved for internal use.
  ...
  #else
    #define AUDIONEEX_API // Untested compiler
  #endif
 #else
...
/// by the recognizer to perform the identification.
...
inline bool IsNull(const IdMatch& res){
    return res.FID==0 && res.IdClass==0 && res.Score==0 && res.Confidence==0;
}
...
/// irrelevant to the identification engine, as long as they are returned in their
/// original formats.
...
public:
	...
	/// fed to the engine until a response is given (whether negative or postive).
    ///
