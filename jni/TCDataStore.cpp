...
/// implement custom drivers for Audioneex data storage. There are several methods
/// to design efficient interfaces for data storages and the ones adopted here are
/// purely for demonstration purposes without claims of fitness for production
/// settings.
...
void TCDataStore::Open(eOperation op, bool use_fing_db, bool use_meta_db, bool use_info_db)
{
    ...
    //       if reranking is never used, and the metadata and info
    //       database are optional. The delta index is only used for
